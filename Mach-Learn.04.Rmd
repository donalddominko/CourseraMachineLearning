...
```{r echo = TRUE, cache = TRUE}
        #obj1 <- tune.rpart(classe ~ ., data = training.01, minsplit = c(5,10,15))
        obj.01 =  tune.rpart(classe ~ ., data = training.01,  minsplit = c(5,10,15))
        summary(obj.01)
        plotcp(obj.01$best.model) # Plot cross validation

        obj.02 =  tune.rpart(classe ~ ., data = training.02,  minsplit = c(5,10,15))
        summary(obj.02)
        plotcp(obj.02$best.model) # Plot cross validation
